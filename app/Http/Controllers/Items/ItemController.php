    ...
    function returnRequiredFormatData($itemMaster, $showWholesalePrice = false)
    {
        ...
        return $itemMaster->map(function ($item) use ($taxList, $unitList, $warehouseId, $showWholesalePrice, $isPermiteToViewPurchasePrice) {
            ...
            if ($showWholesalePrice) {
                $itemsArray['sale_price'] = $item->wholesale_price && $item->wholesale_price > 0
                    ? $item->wholesale_price
                    : ($item->sale_price && $item->sale_price > 0 ? $item->sale_price : 0);
                $itemsArray['is_sale_price_with_tax'] = 0;
                $itemsArray['sale_price_discount'] = 0;
                $itemsArray['sale_price_discount_type'] = 0;
            } else {
                $itemsArray['sale_price'] = $item->sale_price && $item->sale_price > 0
                    ? $item->sale_price
                    : ($item->wholesale_price && $item->wholesale_price > 0 ? $item->wholesale_price : 0);
                $itemsArray['is_sale_price_with_tax'] = 0;
